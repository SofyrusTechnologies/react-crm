...
import {convertArrayToString, momentTimeFormats} from '../Utilities';
...
export default function Leads(props) { 
  ...
  useEffect(() => {        
    if (props.history.location.pathname === '/leads/') getLeads();
    getLeads();
  }, []);
  ...
  const getLeads = () => {
    ...
    axios.get(`${LEADS}`, config)
      .then(res =>  {                
        setApiData(res.data);
        setOpenLeads(res.data.open_leads);
        setClosedLeads(res.data.close_leads);
        setIsLoading(false);        
      });
  }
  ...
  const getTagsAndAssignedUsers = () => {
    ...
    apiData.tags.map(tag => (
      tagsArray.push({value: tag.name, label: tag.name, id: tag.id})
    ))
    setTags(tagsArray);

    let assignedUsers = [];
    apiData.users.map(user => (
      assignedUsers.push({label: user.username, value: user.username, id: user.id})
    ))
        ...
        <tbody>
           {
             leads && leads.map( (lead, index) => {
              return(<tr className="text-center" key={lead.id}>
                 ...
                 <td><a className="text-capitalize mr-1" href={`/users/${lead.created_by.id}/view/`}><img src={lead.created_by.profile_pic} alt={lead.created_by.username} style={{width: "40px", height: "40px"}} title={lead.created_by.email}/></a></td>          
                 ...
                 <td>{
                    ...
                    <td title={momentTimeFormats(lead.created_on)[1]}>{momentTimeFormats(lead.created_on)[0]}</td>
  ...
  const getFilteredLeads = (e) => {                   
    ...
    let config = {
      ...
      params: {
        title: (filterObject.title) ? filterObject.title.trim().toLowerCase() : '',
        source: (filterObject.source)? filterObject.source.trim().toLowerCase() : '',        
        // assigned_to: convertArrayToString(
        //   filterObject.filterUsers ? filterObject.filterUsers.map(user => user.id): []),
        status: (filterObject.status) ? filterObject.status.trim().toLowerCase() : '',
    ...
    axios.get(`${LEADS}`, config
        ...
        setOpenLeads((res.data.open_leads) ? res.data.open_leads: []);
        setClosedLeads((res.data.close_leads) ?  res.data.close_leads: []);
      });
    
  }

  const clearSearchResults = (e) => {
    setOpenLeads(apiData.open_leads);
    setClosedLeads(apiData.close_leads);     
                        ...
                        <div className="card-body">
                          <div className="panel-heading-list card-title text-right">                            
                            ...
                            <span className="filter_toggle">                              
                              <button href="" className="primary_btn" onClick={toggleFilter}><svg className="svg-inline--fa fa-filter fa-w-16" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="filter" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" data-fa-i2svg=""><path fill="currentColor" d="M487.976 0H24.028C2.71 0-8.047 25.866 7.058 40.971L192 225.941V432c0 7.831 3.821 15.17 10.237 19.662l80 55.98C298.02 518.69 320 507.493 320 487.98V225.941l184.947-184.97C520.021 25.896 509.338 0 487.976 0z"></path></svg></button>
                            </span>
                          </div>
                          ...
                          <div className="text-center row marl">
                            ...
                            {
                              (openLeads.open_leads && openLeads.open_leads.length === 0) ? <h3 className="py-2">No active accounts found </h3> : ''
                            }
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="card">
                        <div className="card-body">
                          ...
                          <div className="text-center row marl">
                            ...
                            {
                              (closedLeads.close_leads && closedLeads.close_leads.length === 0) ? <h3 className="py-2">No active accounts found </h3> : ''
                            }
                          </div>
                        </div>
                      </div>
